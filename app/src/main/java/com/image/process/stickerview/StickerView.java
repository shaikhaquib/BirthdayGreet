...
import static com.image.process.designeditor.activity.DesignStudio.saveDrawable;
...
public class StickerView extends FrameLayout {
  ...
  public static float x1_cordinate,x2_coordinate,y1_coordinate,y3_coordinate,rotate,offset_x,offset_y;
  public static String currentModeSelected="";
  ...
  protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
   ...
   // Log.d("onLayout",""+left+" "+right+" "+top+" "+bottom);
    if (changed) {
  ...
  protected void drawStickers(Canvas canvas) {
    ...
    for (int i = 0; i < stickers.size(); i++) {
     // Log.d("get_canvas_width","drawStickers() called");
      ...
      if (sticker != null) {
       ...
       // Log.d("get_canvas_width","canvas_start "+sticker.getCurrentWidth()+" "+sticker.getCurrentHeight()+" "+sticker.getCurrentAngle()+" "+globalX+" "+globalY);
        sticker.draw(canvas);
      }
    }
    ...
    if (handlingSticker != null && !locked && (showBorder || showIcons)) {
      ...
      getStickerPoints(handlingSticker, bitmapPoints);
       ...
       // Drawable dd=handlingSticker.getCurrentScale();
      ...
      if (showBorder) {
        ...
        Log.d("canvas_draw::",x1+" "+y1+" "+x2+" "+y2+" "+x3+" "+y3+" "+x4+" "+y4+"  ");
        x1_cordinate=bitmapPoints[0];
        x2_coordinate=bitmapPoints[2];
        y1_coordinate=bitmapPoints[1];
        y3_coordinate=bitmapPoints[5];

        if(isAdded){
        //  Log.d("added_sticker123","added");
          float img_width=x2_coordinate-x1_cordinate;
          float img_height=y3_coordinate-y1_coordinate;
          String loc=img_width+":"+img_height;
          String overlay=x1_cordinate+":"+y1_coordinate;

        //  imageSize.add(loc);
         // imageCordinate.add(overlay);
         // imageRotate.add("0");
          isAdded=false;
        }
        else{
        //  Log.d("added_sticker123","not added");
        }
      ...
      //  Log.d("rotaion_value","not using: "+rotation);
       // rotate=rotation;
        for (int i = 0; i < icons.size(); i++) {
          ...
          Log.d("added_sticker1234","called added");
        }
      }
    }
  }
  ...
  public boolean onTouchEvent(MotionEvent event) {
    Log.d("Current_Mode1234","onTouchEvent called");
    if (locked) {
    ...
    switch (action) {
      ...
      case MotionEvent.ACTION_POINTER_UP:
        ...
        onStickerOperationListener.onStickerTouchFinish(handlingSticker);
       // Log.d("get_x_y",bitmapPoints[0]+" "+bitmapPoints[1]);
        currentModeSelected="none";
  ...
  protected boolean onTouchDown(@NonNull MotionEvent event) {
    Log.d("Current_Mode1234","onTouchDown called");
  ...
  protected void onTouchUp(@NonNull MotionEvent event) {
    Log.d("Current_Mode1234","onTouchUp called");
    ...
    if (currentMode == ActionMode.DRAG && handlingSticker != null) {
      //Log.d("Current_Mode","onStickerDragFinished");
      if (onStickerOperationListener != null) {
    ...
    onStickerOperationListener.onStickerTouchFinish(handlingSticker);
    currentModeSelected="none";
    Log.d("Current_Mode",currentMode+"");
   // Log.d("get_x_y",bitmapPoints[0]+" "+bitmapPoints[1]);
  ...
  protected void handleCurrentMode(@NonNull MotionEvent event) {
        ...
        if (handlingSticker != null) {
          ...
          if (saveDrawable.contains(handlingSticker.getDrawable())) {
            currentMode = ActionMode.NONE;
          ...
          Log.d("drag_int", s + "drawable--" + handlingSticker.getDrawable());
          switch (currentMode) {
            ...
            case ActionMode.DRAG:
              if (handlingSticker != null) {
                ...
                //Log.d("drag_event_123",constrained+"");
              }
            ...
            case ActionMode.ZOOM_WITH_TWO_FINGER:
              if (handlingSticker != null) {
                ...
                // Log.d("scale_factor_result","aa "+newDistance+" "+oldDistance);
  ...
  public void zoomAndRotateSticker(@Nullable Sticker sticker, @NonNull MotionEvent event) {
    Log.d("sticker_icon", "zoomAndRotateSticker clicked---");
    if (handlingSticker != null) {
      if (saveDrawable.contains(handlingSticker.getDrawable())) {
        currentMode = ActionMode.NONE;
        ...
        if (sticker != null) {
          ...
          moveMatrix.set(downMatrix);
          float div = newDistance / oldDistance;
          // Log.d("zoomRotate",newDistance+" "+oldDistance+" div "+div+" center "+midPoint.x+" "+midPoint.y);
  ...
  protected void constrainSticker(@NonNull Sticker sticker) {
   ...
   // Log.d("constrainSticker",moveX+" "+moveY);
  ...
  @Nullable protected BitmapStickerIcon findCurrentIconTouched() {
    for (BitmapStickerIcon icon : icons) {
      ...
      //Log.d("findCurrentIconTouched","abc"+" "+distance_pow_2+" "+x+" "+y+" "+downX+" "+downY);
  ...
  protected float calculateRotation(float x1, float y1, float x2, float y2) {
    ...
    float abc=(float)radians;
    rotate=abc;
   // Log.d("rotaion_value","using: "+abc+" "+rotate);
  ...
  protected float calculateDistance(float x1, float y1, float x2, float y2) {
    ...
    float srcLen = (float) Math.sqrt(x1 * x1 + y1 * y1);
    float curLen = (float) Math.sqrt(x2 * x2 + y2 * y2);

    float scale = curLen / srcLen;

        // float sub=curLen-srcLen;
        //double sub1=y-x;
       //  Log.d("onStickerTouchFinish123","distance "+x1+" "+y1+"  "+x2+" "+y2+" "+x+" "+y);
  ...
  protected void transformSticker(@Nullable Sticker sticker) {
    ...
    Log.e(TAG, "transformSticker: else");

    Log.d("sticker_icon","transformSticker clicked---");
    ...
    if (width < height) {
      ...
      Log.e("zoomRotate", " start "+scaleFactor+"div "+(scaleFactor/2f));
  ...
  public boolean remove(@Nullable Sticker sticker) {
    //Log.d("onActivityResult","remove:"+sticker.getDrawable());
    Log.d("sticker_icon","remove clicked---");
    if(saveDrawable.contains(handlingSticker.getDrawable())){
      currentMode = ActionMode.NONE;
      ...
      if (stickers.contains(sticker)) {
        ...
        //  getDeletedDrawable(sticker.toString());
        invalidate();
  ...
  @NonNull public StickerView addSticker(@NonNull Sticker sticker) {
   ...
   // Log.d("add_sticker_view",x1_cordinate+" "+x2_coordinate+" "+y1_coordinate+" "+y3_coordinate);
   // drawableList.add(sticker.toString());
   // drawableName.add(from);
    isAdded=true;
  ...
  protected void addStickerImmediately(@NonNull Sticker sticker, @Sticker.Position int position) {
    ...
    if (sticker.getDrawable()!=null) {
      ...
      //    Log.d("zoomRotate",scaleFactor+" add div "+(scaleFactor/2)+" w "+(getWidth()/2)+" h "+(getHeight()/2));
      //Log.d("addStickerImmediately",widthScaleFactor+" "+heightScaleFactor+" "+scaleFactor);
  ...
  protected void setStickerPosition(@NonNull Sticker sticker, @Sticker.Position int position) {
   ...
   // Log.d("setStickerPosition",offsetX+" "+offsetY);
   ...
   // Log.d("offset_value4445",offsetX+" "+offsetY);
  ...
  public interface OnStickerOperationListener {
    ...
    void onStickerTouchFinish(@NonNull Sticker sticker);
